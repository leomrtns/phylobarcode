...
def run_extract_operons_from_fasta (args):
    ...
    task_extract_riboproteins.extract_operons_from_fasta (coord_tsvfile = args.coords, merge_tsvfile = args.tsv, 
            ...
            most_common_mosaics = args.most_common, border = args.border, nthreads=args.nthreads, scratch=args.scratch)
...
def main():
    ...
    '''
    up_findp = subp.add_parser('extract_coordinates_from_gff', help=this_help, description=this_help + extra_help, parents=[parent_parser],
            formatter_class=argparse.RawTextHelpFormatter, epilog=epilogue)
    ...
    '''
    up_findp = subp.add_parser('extract_operons_from_fasta', help=this_help, description=this_help + extra_help, parents=[parent_parser],
            formatter_class=argparse.RawTextHelpFormatter, epilog=epilogue)
    up_findp.add_argument('tsv', help="tsv file with file matches between fasta and GFF3 (required)")
    up_findp.add_argument('-a', '--fasta', metavar="<dir>", required=True, 
